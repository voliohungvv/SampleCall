...
import com.volio.vn.callscreen.extensions.getContrastColor
...
import com.volio.vn.callscreen.extensions.letterBackgroundColors
import com.volio.vn.callscreen.extensions.queryCursor
...
class SimpleContactsHelper(val context: Context) {
    ...
    fun getContactNames(favoritesOnly: Boolean): List<SimpleContact> {
    ...
    /*fun setPhotoUriFromPhoneNumber(
        number: String,
        pathImage: String,
        onSuccess: () -> Unit,
        onFailure: () -> Unit
        ...
        val data: Cursor = context.getContentResolver().query(lookupUri, projection, null, null, null)!!
        data.moveToFirst()
        try {
        ...
        try {
            ...
            contentValues.put(PhoneLookup.PHOTO_URI, uriImage.toString())
            val rowEffect = context.contentResolver.update(uri, contentValues, null, null)
            if (rowEffect > 0) {
                onSuccess()
            } else {
                onFailure()
    ...
    fun setDisplayPhotoByRawContactId(phoneNumber: String, bmp: Bitmap): Boolean {
        val lookupUri = Uri.withAppendedPath(PhoneLookup.CONTENT_FILTER_URI, phoneNumber)
        ...
        data.moveToFirst()
        val rawContactId = data.getLong(0)
        ...
        try {
            val afd: AssetFileDescriptor? =
                context.contentResolver.openAssetFileDescriptor(pictureUri, "rw")
